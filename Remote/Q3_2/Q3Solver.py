# -*- coding: utf-8 -*-
"""
Q3Solver.py — Problem 3 (final, with cooperative masking & monotone convergence plot)
...
2) Coordinate Descent micro-tuning (default ON, disable via --no_cd)
3) Cooperative masking across bombs: per-point OR across bombs ANDed over cylinder samples
4) Time grid only on union of effective intervals
5) Cheap necessary test (LOS→TRUE_TARGET distance to cloud center) before expensive cone check
6) Two-level parallelism (outer candidates + inner time-chunks), Windows-friendly
7) Rich [Q3][INFO] logs; Excel export `result1.xlsx`
8) Convergence plot `q3_convergence.png` (English labels; clean style, monotonically non-decreasing)
9) Stage2 batching + candidate cap + quiet workers to avoid stdout congestion
"""
...
# ------------------ cooperative cone test (pointwise OR across bombs) ------------------
...
def eval_chunk_union(args):
    ...
    Cheap necessary test before cone check based on LOS distance to cloud center.
    """
...
def save_convergence_plot(history, path="q3_convergence.png", shade=False):
    ...
    We draw the prefix-maximum of the recorded best values to guarantee monotonicity.
    """
    if not history:
        return
    xs = [h["idx"] for h in history]
    ys = np.array([h["best"] for h in history], dtype=float)
    # double insurance: enforce non-decreasing series
    ys = np.maximum.accumulate(ys)

    # optional phase ranges (not shaded by default)
    ...
    for rd in range(1, rounds+1):
        ...
        for key in keys:
            ...
            for sgn in (+1, -1):
                ...
                # update global best & record monotone history
...
def main():
    ...
    info("="*90)
    info("Start Q3 optimization: two-stage + coordinate descent + cooperative masking")
            ...
            with ProcessPoolExecutor(max_workers=args.search_workers) as pool:
                ...
                while futs:
                    for fut in as_completed(list(futs.keys())):
                        ...
                        # update global best and record monotone history

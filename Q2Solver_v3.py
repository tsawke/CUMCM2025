...
def pattern_search(x0: np.ndarray,
    ...
    并行评估所有 8 个方向（4 维 × ±）。
    返回：best_val, best_x, expl_pos, hit_time
    """
...
def de_opt(pop: int, iters: int, eval_kwargs: dict, workers: int = None,
           ...
           init_seeds: List[np.ndarray] = None) -> Tuple[float, np.ndarray, np.ndarray, float]:
    """
...
def hybrid_opt(pop: int, iters: int, workers: int,
    ...
    for rank,(sec0, x0, expl0, hit0) in enumerate(top, start=1):
        ...
        best_sec, best_x, best_expl, best_hit = pattern_search(
            x0, steps,
...
def generate_convergence_plot(history, algorithm_name, best_value):
    """
    生成优化算法收敛曲线可视化
    :param history: 收敛历史数据
    :param algorithm_name: 算法名称
    :param best_value: 最优值
    """
    try:
        # 创建图像
        ...
        ax1.plot(iterations, history, 'b-', linewidth=2, alpha=0.8, label='Convergence')
        ax1.axhline(y=best_value, color='r', linestyle='--', linewidth=2,
                   label=f'Final Best: {best_value:.6f}s')
        ax1.fill_between(iterations, 0, history, alpha=0.3, color='blue')

        ax1.set_xlabel('Iteration')
        ax1.set_ylabel('Occlusion Duration (s)')
        ax1.set_title(f'{algorithm_name.upper()} Convergence Curve')
        ax1.legend()
        ...
        if len(history) > 1:
            ...
            improvement = [(val - initial_value) for val in history]
            ax2.plot(iterations, improvement, 'g-', linewidth=2, alpha=0.8, label='Improvement')
            ...
            ax2.fill_between(iterations, 0, improvement, alpha=0.3, color='green')
        ...
        # 添加统计信息
        stats_text = ".2f"".2f"".4f"        fig.suptitle(f'Q2 Optimization Results - {algorithm_name.upper()} Algorithm\n{stats_text}',
                    fontsize=14, fontweight='bold')

        plt.tight_layout()
        plt.savefig(f'Q2_{algorithm_name}_Convergence.png', dpi=300, bbox_inches='tight')
        plt.show()

        print(f"[Q2] Convergence plot saved as Q2_{algorithm_name}_Convergence.png")

...
def _print_init_bar(step: int, total: int):
    """单行进度条，不在子进程打印；不反复刷屏"""
...
# =========================
_total_init_steps = 3
_print_init_bar(1, _total_init_steps)

Q1_IMPORTED = False
try:
    import Q1Solver as Q1
    _print_init_bar(2, _total_init_steps)
    g = getattr(Q1, "g", 9.8)
    ...
    Q1_IMPORTED = True
    _print_init_bar(_total_init_steps, _total_init_steps)
except Exception:
    # 不额外输出任何初始化文本，这里也只更新一次进度条
    _print_init_bar(2, _total_init_steps)
    # ------- 回退实现（与 Q1 严格几何判据一致） -------
    g = 9.8
    MISSILE_SPEED = 300.0
...
# >>> 新增：投放点相关 <<<
def drop_point_from_params(heading, speed, drop):
    """由参数算投放点（FY1 水平等速、等高度）"""
    ...
    for k in range(1, iters+1):
        ...
        for i in range(n_chains):
            ...
            if np.any(better_mask):
                ...
                j_abs = start + j_rel
                X[i] = np.array([cand_tasks[j_abs][0], cand_tasks[j_abs][1], cand_tasks[j_abs][2], cand_tasks[j_abs][3]], dtype=float)
...
def save_report(filename: str, algo: str,
                ...
                params: Dict = None):
    # >>> 新增：计算投放点，并写入报告 <<<
    heading, speed, drop, fuse = float(best_x[0]), float(best_x[1]), float(best_x[2]), float(best_x[3])
    drop_pos = drop_point_from_params(heading, speed, drop)
    # >>> 新增结束 <<<
    ...
    with open(filename, "w", encoding="utf-8") as f:
        ...
        # >>> 新增一行：Drop point（放在 Explosion point 前） <<<
        f.write(f"Drop point = ({drop_pos[0]:.6f}, {drop_pos[1]:.6f}, {drop_pos[2]:.6f})\n")
        # >>> 新增结束 <<<
...
def main():
    ...
    for algo in algos:
        ...
        # >>> 新增：控制台也打印投放点 <<<
        drop_pos_print = drop_point_from_params(best_x[0], best_x[1], best_x[2])
        print(f"[Best-{algo}] drop=({drop_pos_print[0]:.2f},{drop_pos_print[1]:.2f},{drop_pos_print[2]:.2f})")
        # >>> 新增结束 <<<
    ...
    if overall_rec is not None:
        ...
        # >>> 新增：总体最佳也打印投放点 <<<
        drop_pos_best = drop_point_from_params(best_x[0], best_x[1], best_x[2])
        print(f"     drop=({drop_pos_best[0]:.2f},{drop_pos_best[1]:.2f},{drop_pos_best[2]:.2f})")
        # >>> 新增结束 <<<
        ...
        with open("Q2_All_Summary.txt", "w", encoding="utf-8") as f:
            ...
            # >>> 新增：Summary 里也写入投放点 <<<
            f.write(f"Drop point = ({drop_pos_best[0]:.6f}, {drop_pos_best[1]:.6f}, {drop_pos_best[2]:.6f})\n")
            # >>> 新增结束 <<<
            f.write(f"Explosion point = ({best_expl[0]:.6f}, {best_expl[1]:.6f}, {best_expl[2]:.6f})\n")

...
Highlights:
- Cooperative masking across bombs (per-point OR, all-points AND)
...
# Geometry/physics from Q1 (place next to this file)
...
def cone_points_in(m_pos: np.ndarray, c: np.ndarray, pts: np.ndarray, R: float) -> np.ndarray:
    """(old) Infinite-cone inclusion. Keep for compatibility."""
    a = c - m_pos
    d2 = float(np.dot(a, a))
    b = pts - m_pos[None, :]
    dot = b @ a
...
def segment_points_mask(m_pos: np.ndarray, pts: np.ndarray, c: np.ndarray, R: float) -> np.ndarray:
    """# NEW: Physically correct: does the segment M→P intersect the sphere (C,R) or endpoints inside? (vectorized)"""
    d = pts - m_pos[None, :]                 # (N,3)
    f = m_pos - c                            # (3,)
    a = np.sum(d*d, axis=1)                  # (N,)
    b = 2.0 * np.sum(f[None,:] * d, axis=1)  # (N,)
    csc = float(np.dot(f, f) - R*R)          # scalar

    disc = b*b - 4.0*a*csc
    mask_disc = disc >= 0.0

    # roots only where a>0 and disc>=0
    good = mask_disc & (a > 0.0)
    ...
    # endpoints inside also counts
    inside_M = (np.dot(m_pos - c, m_pos - c) <= R*R)
    inside_P = (np.sum((pts - c[None,:])**2, axis=1) <= R*R)
...
def eval_chunk_union_fixed(args):
    ...
    """
    (idx0, t_chunk, bombs, t_expls, pts, margin, block, chunk_id, total_chunks, inner_log_every, masking_mode) = args
    n = len(t_chunk)
    ...
    for i, t in enumerate(t_chunk):
        ...
        for bi, (expl, texp) in enumerate(zip(bombs, t_expls)):
            ...
            # FIXED: Removed the restrictive "cheap necessary test" that was causing issues
            # Directly compute masking without pre-filtering
...
def evaluate_strategy_fixed(params: Dict[str, Any], sim: Dict[str, Any]):
    ...
    # Constraints at drop/explosion time
    ...
    if sim.get("timegrid_mode", "union") == "union":
        intervals = []
        for te, expl in zip(t_expls, bombs_expl):
            a = float(te)
            b_list = [te + Q1.SMOG_EFFECT_TIME, hit_time]
            if Q1.SMOG_SINK_SPEED > 0.0:
                b_list.append(te + float(expl[2]) / float(Q1.SMOG_SINK_SPEED))
            b = float(min(b_list))
            if b > a + 1e-12:
        ...
        t0 = min(t_expls)
        # conservative full window but still respect ground until max expl_z/sink
        max_te = max(t_expls)
        max_expl_z = max(float(e[2]) for e in bombs_expl)
        b_ground = (max_te + max_expl_z / float(Q1.SMOG_SINK_SPEED)) if Q1.SMOG_SINK_SPEED > 0 else (max_te + Q1.SMOG_EFFECT_TIME)
        t1 = min(max_te + Q1.SMOG_EFFECT_TIME, hit_time, b_ground)
        if t1 <= t0 + 1e-12:
            return 0.0, [0.0, 0.0, 0.0], None
    ...
    if sim["inner_log"]:
        info(f"Candidate done: union={total_seconds:.6f}s, per-bomb={secs_each}")
...
# ------------------ Convergence plot (monotone) ------------------
def save_convergence_plot(history, path="q3_convergence.png", shade=False):
    if not history:
        return
    xs = [h["idx"] for h in history]
    ys = np.array([h["best"] for h in history], dtype=float)
    ys = np.maximum.accumulate(ys)  # monotone
    phases = [h["phase"] for h in history]
    def phase_range(p):
        idxs = [i for i, ph in enumerate(phases) if ph == p]
        return None if not idxs else (xs[idxs[0]], xs[idxs[-1]])
    p1 = phase_range("stage1"); p2 = phase_range("stage2"); p3 = phase_range("cd")
    plt.figure(figsize=(10, 5), dpi=150)
    plt.plot(xs, ys, lw=2)
    if shade:
        def shade_span(rng, color, label):
            if rng: plt.axvspan(rng[0], rng[1], alpha=0.08, color=color, label=label)
    ...
    if len(ys):
        ...
        plt.annotate(f"best={ys[i_best]:.3f}s @#{xs[i_best]}",
                     xy=(xs[i_best], ys[i_best]), xytext=(6, 10), textcoords="offset points", fontsize=9)
    plt.grid(True, alpha=0.25)
    handles, labels = plt.gca().get_legend_handles_labels()
    if handles: plt.legend(frameon=False, loc="best")
...
def coordinate_descent(seed_params: Dict[str, Any],
                       ...
                       global_best_total: List[float]):
    def within(v, low, high): return max(low, min(high, v))
    def repair_time_vars(p):
        t1, t2, t3 = p["drops"]
        g12 = max(1.0, t2 - t1); g23 = max(1.0, t3 - t2)
        t1 = max(0.0, t1); t2 = t1 + g12; t3 = t2 + g23
        p["drops"] = [t1, t2, t3]
        for i in range(3):
            f = max(0.1, p["fuses"][i])
    ...
    for rd in range(1, rounds+1):
        improved = False; info(f"[CD] round {rd} start")
        for key in keys:
            ...
            for sgn in (+1, -1):
                ...
                global_best_total[0] = max(global_best_total[0], val, best_val)
                conv_hist.append(dict(idx=eval_counter[0], best=global_best_total[0], phase="cd"))
        if not improved:
...
def main():
            ...
            with ProcessPoolExecutor(max_workers=args.search_workers) as pool:
                ...
                while futs:
                    for fut in as_completed(list(futs.keys())):
                        ...
                        global_best_total[0] = max(global_best_total[0], (nonlocal_best["total"] if nonlocal_best else -1e9))
                        conv_hist.append(dict(idx=eval_counter[0], best=global_best_total[0], phase=phase_label))
    ...
    if args.stage2_enable:
        ...
        sim_stage2 = dict(sim); sim_stage2["inner_log"] = False
        t2_a = time.time()
    ...
    if args.cd_enable:
        ...
        best_params_after_cd, best_val_after_cd = coordinate_descent(
            seed_params=dict(best["params"]), sim=sim_cd, steps=steps, bounds=bounds,

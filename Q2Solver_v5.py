...
多算法版：Hybrid / DE / PSO / SA / Pattern
并行评估 + 两阶段精度（粗筛/终评）+ 兼容 Q1Solver 的几何判据（导入失败有回退实现）
...
import argparse
import numpy as np
import matplotlib.pyplot as plt  # 修复：需要显式导入
...
FUSE_MIN, FUSE_MAX = 0.0, 18.0     # 太长会落地失效
...
# 命中时刻（常量）
_HIT_TIME_CONST = float(np.linalg.norm(M1_INIT - FAKE_TARGET_ORIGIN) / MISSILE_SPEED)
...
def quick_hard_filters(heading, speed, drop, fuse):
    ...
    t0 = drop + fuse
    t1 = min(t0 + SMOG_EFFECT_TIME, _HIT_TIME_CONST)
    if expl_pos[2] <= 0.0:  # 爆在地面/地下
        return False, t0, t1, _HIT_TIME_CONST, expl_pos
    if t1 <= t0:            # 没有有效时间窗
        return False, t0, t1, _HIT_TIME_CONST, expl_pos
    return True, t0, t1, _HIT_TIME_CONST, expl_pos
...
# 采样点缓存，避免重复构造
_pts_cache: Dict[Tuple[int,int], np.ndarray] = {}

def _get_pts(nphi: int, nz: int) -> np.ndarray:
    key = (int(nphi), int(nz))
    arr = _pts_cache.get(key)
    if arr is None:
        arr = PreCalCylinderPoints(nphi, nz, dtype=float)
        _pts_cache[key] = arr
    return arr
...
def pattern_search(x0: np.ndarray,
    ...
    """
    简单模式/坐标搜索（无导数）：沿每个坐标±方向试探，若改进则接受；否则步长缩小。
    并行评估所有 8 个方向（4 维 × ±）。
    返回：best_val, best_x, expl_pos, hit_time, history
    """
...
def de_opt(pop: int, iters: int, eval_kwargs: dict, workers: int = None,
    ...
    返回：(gbest_score, gbest, gbest_expl, hit_time, convergence_history)
    """
    rng = np.random.default_rng()
    # 初始化种群
    X = []
...
def pso_opt(pop: int, iters: int, eval_kwargs: dict, workers: int = None,
            ...
            init_seeds: List[np.ndarray] = None) -> Tuple[float, np.ndarray, np.ndarray, float, List[float]]:
    """
    粒子群优化（改进：角度取模、越界反弹），外层并行评估
    返回：(Gscore, Gbest, Gexpl, hit_time, convergence_history)
    """
    rng = np.random.default_rng()
    # 初始化
    X, V = [], []
    ...
    for it in range(iters):
        ...
        for i in range(pop):
            ...
            # 对非角度维超界做反弹
            for j,(mn,mx) in enumerate([(HEADING_MIN,HEADING_MAX),(SPEED_MIN,SPEED_MAX),(DROP_MIN,DROP_MAX),(FUSE_MIN,FUSE_MAX)]):
                if j==0:  # 角度不用反弹
                    continue
...
def sa_opt(x0: np.ndarray, eval_kwargs: dict,
    ...
    """
    模拟退火（单轨迹），对困难非凸问题有时能翻山。
    返回：(best_f, best_x, best_expl, hit, convergence_history)
    """
    rng = np.random.default_rng()
    x = clamp_params(x0.copy())
...
# =========================
# 5) HYBRID：LHS 粗筛 -> TopK 局部搜 -> DE 全局收敛 -> 终评
# =========================
...
def hybrid_opt(pop: int, iters: int, workers: int,
    ...
    """
    混合策略：
      1) LHS 采样 pop 份，用粗精度并行评估，取 Top-K；
      2) 对 Top-K 分别做 Pattern Search 局部精修（粗精度）；
      3) 将所有精修后的解作为种子，执行 DE（粗精度）；
      4) 将 DE 输出的前若干名，改用终评精度复评，给出最终 Top-N 与最优。
    返回：best_val, best_x, best_expl, hit_time, top_list
    """
    ...
    print("[HYBRID] 阶段2：Top-K 局部 Pattern Search 精修（并行每个起点的坐标搜索）")
    refined = []
    # 串行起 K 次局部搜索（每次内部的坐标评估是并行的）
    for rank,(sec0, x0, expl0, hit0) in enumerate(top, start=1):
        print(f"  -> Pattern Search 起点#{rank}  初值={sec0:.6f}")
        steps = np.array([0.1, 5.0, 0.5, 0.5], dtype=float)  # 初始步长（角/速/时/时）
    ...
    best_de, x_de, expl_de, hit_de, history_de = de_opt(
        pop=pop, iters=iters,
        eval_kwargs=dict(dt=dt_coarse, nphi=nphi_coarse, nz=nz_coarse, margin=EPS, block=8192),
    ...
    # 组装候选（DE 的全体种群不可得，这里用 refined 的前若干 + DE 最优）
    candidates = []
    ...
    best = finals[0]
    return best[0], best[1], best[2], best[3], finals[:min(10,len(finals))]
...
def save_report(filename: str,
    ...
    with open(filename, "w", encoding="utf-8") as f:
        f.write("="*80 + "\n")
        f.write("Q2 Optimization Results Report\n")
        f.write("="*80 + "\n\n")
...
def generate_convergence_plot(history, algorithm_name, best_value):
    """
    生成优化算法收敛曲线可视化
    :param history: 收敛历史数据（list[float]）
    :param algorithm_name: 算法名称
    :param best_value: 最优值（终评或最终）
    """
    try:
        if history is None or len(history) == 0:
            print("[Q2] No convergence history to plot.")
            return
        ...
        ax1.plot(iterations, history, linewidth=2, alpha=0.9, label='Best-so-far')
        ax1.axhline(y=best_value, color='r', linestyle='--', linewidth=2,
                    label=f'Final Best: {best_value:.6f}s')
        ax1.set_xlabel('Iteration')
        ax1.set_ylabel('Occlusion Duration (s)')
        ax1.set_title(f'{algorithm_name.upper()} Convergence')
        ax1.legend()
        ax1.grid(True, alpha=0.3)

        # 改进曲线（相对初始）
        imp = [h - initial_value for h in history]
        ax2.plot(iterations, imp, linewidth=2, alpha=0.9, label='Improvement vs. initial')
        ax2.axhline(y=(best_value - initial_value), color='r', linestyle='--', linewidth=2,
                    label=f'Final Improvement: {best_value - initial_value:.6f}s')
        ...
        # 总标题（包含统计信息）
        stats_text = f'init={initial_value:.6f}s | last={final_value:.6f}s | max={max_value:.6f}s | Δ={improvement:.6f}s'
        fig.suptitle(f'Q2 Optimization Results - {algorithm_name.upper()} \n{stats_text}',
                     fontsize=14, fontweight='bold')

        plt.tight_layout()
        out_name = f'Q2_{algorithm_name}_Convergence.png'
        plt.savefig(out_name, dpi=300, bbox_inches='tight')
        plt.show()

        print(f"[Q2] Convergence plot saved as {out_name}")

    except Exception as e:
        print(f"[Warning] Failed to generate convergence plot: {e}")

def main():
    ap = argparse.ArgumentParser("Q2 Optimizer")
    ...
    if args.algo == "hybrid":
        ...
        # Hybrid 内部聚合多阶段，不单独返回 history，这里不画或可自定义组合历史
        history = None
    elif args.algo == "de":
    ...
    print(f"[Best] h={best_x[0]:.6f} rad, v={best_x[1]:.3f} m/s, drop={best_x[2]:.3f} s, fuse={best_x[3]:.3f} s")

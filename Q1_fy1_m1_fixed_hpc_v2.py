# -*- coding: utf-8 -*-
# Q1_fy1_m1_fixed_extreme.py
# 超高精度 + 快速早停 + 强化并行（严格圆锥判据）
...
def precompute_cylinder_points(n_phi, n_z_side, dtype=np.float64):
    B = CYL_BASE_CENTER.astype(dtype)
...
def explosion_point(heading_rad, t_drop, fuse_dt, dtype=np.float64):
    drop_pos, uav_v = uav_state_horizontal(t_drop, FY1_INIT.astype(dtype), dtype(UAV_SPEED), heading_rad)
    expl_xy = drop_pos[:2] + uav_v[:2] * dtype(fuse_dt)
    expl_z = drop_pos[2] - dtype(0.5)*dtype(g)*(dtype(fuse_dt)**2)
    return np.array([expl_xy[0], expl_xy[1], expl_z], dtype=dtype)

# —— 快速早停版：按点块判定，无需整批向量化到底 —— #
def _cone_all_points_in(M, C, P, r_cloud=CLOUD_R, margin=1e-12, block=8192):
    v = C - M
    L = np.linalg.norm(v)
    if L <= 1e-12 or r_cloud >= L:
        return True
...
def _eval_chunk(args):
    ...
    for i, t in enumerate(t_chunk):
        ...
        C = np.array([expl_pos[0], expl_pos[1], expl_pos[2] - CLOUD_SINK * max(0.0, float(t) - t_expl)], dtype=expl_pos.dtype)
        out[i] = _cone_all_points_in(M, C, pts, r_cloud=CLOUD_R, margin=margin, block=block)
    return idx0, out

def compute_q1_extreme(
    dt=0.0005, nphi=960, nz=13,
    backend="process", workers=None, chunk=800,
    fp32=False, margin=1e-12, block=8192
):
    ...
    expl_pos = explosion_point(heading, DROP_T, FUSE_DT, dtype=dtype)
    if expl_pos[2] <= 0:
    ...
    t0, t1 = EXPL_T, min(EXPL_T + CLOUD_EFFECT, hit_time)
    t_grid = np.arange(t0, t1 + 1e-12, dt, dtype=dtype)
    ...
    with pool_cls(max_workers=workers) as pool:
        futs = {pool.submit(_eval_chunk, (idx, c, expl_pos, EXPL_T, pts, margin, block)): idx for idx, c in chunks}
        done = 0
...
def main():
    ...
    ap.add_argument("--margin", type=float, default=1e-12, help="圆锥判据余量（防止边界抖动）")

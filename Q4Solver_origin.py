...
def main():
    ...
    # ★★★ 修复：为 SA 阶段并行精评先计算 workers2 / intra2
    workers2, intra2 = _auto_balance(args.workers, args.intra_threads, args.backend, total_tasks=sa_batch)

    # 为了吃满核，对 proposals 的精确协同评分用进程池/线程池
    if args.backend == "process":
    ...
    # 注意：x_best 含 FY1 的分量，但 _tri_from_x 只对 FY2/FY3 生效
    def _x_of(tri):
        return np.array([tri[0]["T_e"], tri[1]["T_e"], tri[2]["T_e"],
                         tri[0]["speed"], tri[1]["speed"], tri[2]["speed"]], dtype=float)

...
try:
    ...
    Q1_IMPORTED = True
    print("[Init] Geometry backend: external(Q1Solver)")
except Exception as e:
    print(f"[Init] Geometry backend: internal(fallback)  detail={e}")
    # ------- 回退实现（与 Q1 严格几何判据一致） -------
    g = 9.8
    MISSILE_SPEED = 300.0
    SMOG_R = 10.0
...
def heuristic_seeds(k: int = 8) -> List[np.ndarray]:
    """
    物理可行的热启动：朝原点、v≈120、题1标注(drop=1.5,fuse=3.6)等的扰动
    """
    rng = np.random.default_rng()
    seeds = []
...
def sa_opt(x0: np.ndarray, eval_kwargs: dict,
           T0: float = 1.0, Tend: float = 1e-3, iters: int = 8000,
           step_scale: np.ndarray = None, no_improve_patience: int = 2000
           ) -> Tuple[float, np.ndarray, np.ndarray, float, List[float]]:
    ...
    for k in range(1, iters+1):
        ...
        step = (np.array([0.2, 10.0, 1.0, 1.0]) if step_scale is None else step_scale) * max(T, 1e-3)
        ...
        if fc >= f or rng.random() < math.exp((fc - f)/max(T,1e-9)):
            x, f, expl, hit = cand, fc, explc, hitc
            if f > best_f:
                best_f, best_x, best_expl = f, x.copy(), expl
                last_improve = k
        if k % 200 == 0:
        ...
        if k - last_improve >= no_improve_patience:
            print(f"[SA] early stop at {k}, no improvement for {no_improve_patience} steps.")
            break
    return best_f, best_x, best_expl, hit, hist
...
def hybrid_opt(pop: int, iters: int, workers: int, topk: int,
    ...
    with ProcessPoolExecutor(max_workers=workers) as pool:
        ...
        for fut in as_completed(futs):
            ...
            if done % stride == 0 or done == len(seeds):
                ...
                print(f"    [HYBRID] LHS 进度 {int(100*done/len(seeds))}%  best_so_far={best_so_far:.6f}")

    vals.sort(key=lambda x: x[0], reverse=True)
    top = vals[:topk]
    print(f"[HYBRID] 粗筛 Top-{topk} 最佳 = {top[0][0]:.6f} s")

    print("[HYBRID] 阶段2：Top-K Pattern 精修")
    ...
    seeds2 = [r[1] for r in refined[:min(len(refined), max(4, topk//2))]] + heuristic_seeds(k=4)
    ...
    for algo in algos:
        ...
        if algo == "hybrid":
            ...
            best_val, best_x, best_expl, hit_time, history = sa_opt(
                x0=x0,
                eval_kwargs=dict(dt=args.dt_coarse, nphi=args.nphi_coarse, nz=args.nz_coarse, margin=EPS, block=8192),
                T0=1.0, Tend=1e-3, iters=args.sa-iters if hasattr(args, "sa-iters") else args.sa_iters,
                step_scale=np.array([0.2, 10.0, 1.0, 1.0]),
                no_improve_patience=max(1000, args.sa_iters//3 if hasattr(args, "sa_iters") else 2500)
            )
            sec_tmp, expl_tmp, hit_tmp = evaluate_occlusion(*best_x, dt=args.dt_final, nphi=args.nphi_final, nz=args.nz_final)
            best_val, best_expl, hit_time = sec_tmp, expl_tmp, hit_tmp
        else:  # pattern
